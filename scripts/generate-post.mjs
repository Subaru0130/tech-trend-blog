...
async function generateArticle(topic) {
  console.log(`\n噫 Starting Generation Pipeline for: ${topic}`);
    ...
    **Structure & Requirements (MDX)**:
    1.  **Frontmatter**:
        - title: "SEO Optimized Title"
          - **MUST FOLLOW THIS PATTERN**: "縲慎opic縲然ecommendation Ranking N Selection縲殖randA vs BrandB vs BrandC縲・
          - **CRITICAL**: The TOPIC (${topic}) MUST be the VERY FIRST word.
          - **Example**: "縲仙刈貉ｿ蝎ｨ縲代♀縺吶☆繧∽ｺｺ豌励Λ繝ｳ繧ｭ繝ｳ繧ｰ5驕ｸ縲占ｱ｡蜊ｰ繝ｻ繝繧､繝九メ繝ｻ繝代リ繧ｽ繝九ャ繧ｯ蠕ｹ蠎墓ｯ碑ｼ・・
          - **Example**: "縲舌ラ繝ｩ繧､繝､繝ｼ縲醍ｾ主ｮｹ蟶ｫ縺翫☆縺吶ａ5驕ｸ縲舌ヱ繝翫た繝九ャ繧ｯ繝ｻ繝繧､繧ｽ繝ｳ繝ｻ繝ｪ繝輔ぃ豈碑ｼ・・
          - Include concrete brand names of the top 3 products.
        - date: ${(new Date()).toISOString().split('T')[0]}
        - description: "Generate a compelling, click-worthy meta description (max 120 chars). Mention the benefits of reading this verification. Ex: '蠖ｻ蠎墓ｯ碑ｼ・0驕ｸ...邨占ｫ悶・縺薙ｌ縺'"
    ...
    5.  **Buying Guide**: 
     ...
     7.  **The Ranking (1 to 5)**:
        - **IMPORTANT**: Section Header MUST be "### 隨ｬN菴・ Product Name" (Do NOT use "Rank N").
        ...
        - **Props**:
          ...
          - bestFor="Target Persona (e.g. '蟄占ご縺ｦ荳門ｸｯ', '譁咏炊螂ｽ縺・)"
        ...
        **CRITICAL REQUIREMENT: AFTER EACH <RankingCard/>, you MUST write a "360ﾂｰ Expert Analysis" block.**
        ...
        **Structure for each product (REQUIRED):**
        ...
        2. **Angle 1: Situation Fit ("How it changes your life")**
           - Headline: \`#### 桁 縲千函豢ｻ縺悟､峨ｏ繧九曾${Target Persona}縺ｧ縺ｮ菴ｿ縺・ｿ・慍\`
     ...
     8.  **Comparison Table**:
        ...
        - **Example**:
          \`\`\`js
          specLabels: { size: "繧ｵ繧､繧ｺ", weight: "驥埼㍼", power: "豸郁ｲｻ髮ｻ蜉・, cost: "繧ｳ繧ｹ繝・ }
          products={[
            { rank: 1, name: "陬ｽ蜩∝錐", image: "...", asin: "B00xxxx", specs: { size: "Compact", weight: "500g", power: "1200W", cost: "笳・ } },
          ]}
          \`\`\`
    ...
    10. **Conclusion**: 
        ...
        - **CRITICAL**: Every time you mention a product name here, you **MUST** format it as a link to Amazon.
          - **Good**: "髱咎浹諤ｧ縺ｪ繧・[繝繧､繝九メ HD-RX500A](SEARCH:Dainichi HD-RX500A) 縺後♀縺吶☆繧√〒縺吶・
  ...
  if (!result.candidates || result.candidates.length === 0 || !result.candidates[0].content) {
    console.error("笶・Gemini Generation Failed: No candidates returned.");
  ...
  while ((fallbackMatch = fallbackRegex.exec(mdxContent)) !== null) {
    ...
    if (!urlMap.has(url)) {
      console.log("  笞・・Image found without product name association. Using generic name.");
...
async function main() {
  const topic = process.argv[2] || '蜉貉ｿ蝎ｨ';
  ...
  try {
    ...
    console.log("笨・Generation & Verification Complete.");
  } catch (e) {
    console.error("笶・GENERATION / VERIFICATION FAILED:", e);
    process.exit(1);
  } finally {
    // Cleanup ExifTool
