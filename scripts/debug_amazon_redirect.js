...
(async () => {
    console.log("噫 Starting Amazon Redirect Debug with FIX...");
    ...
    try {
        ...
        console.log(`1・鞘Ε  Navigating to Kakaku product page: ${TARGET_URL}`);
        ...
        console.log(`2・鞘Ε  Found Amazon Shop Link: ${amazonInfo.url}`);
        ...
        console.log(`3・鞘Ε  Found Forwarder Link: ${forwarderUrl}`);
        console.log(`竢ｳ Attempting redirect with RELAXED wait condition...`);
        ...
        try {
            ...
            console.log(`   笞・・Navigation warning (ignored if URL is correct): ${e.message}`);
        }

        const finalUrl = page.url();
        console.log(`4・鞘Ε  Final URL: ${finalUrl}`);

        if (finalUrl.includes('amazon.co.jp')) {
            console.log("笨・SUCCESS! Reached Amazon.");

            // Extract ASIN
            const asinMatch = finalUrl.match(/\/dp\/([A-Z0-9]{10})/);
            if (asinMatch) {
                console.log(`   逃 ASIN Extracted: ${asinMatch[1]}`);
            }

            // Verify content is accessible (not robot block)
            const title = await page.evaluate(() => document.title);
            console.log(`   塘 Page Title: ${title}`);
        } else {
            console.log("笶・FAILED. Did not reach Amazon.");
        }

    } catch (e) {
        console.error(`笶・Error: ${e.message}`);
    } finally {
        await browser.close();
    }
})();
