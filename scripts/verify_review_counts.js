...
const PRODUCTS = [
    "Sony WF-1000XM5",
    "Apple AirPods Pro (隨ｬ2荳紋ｻ｣)",
    "Anker Soundcore Liberty 4"
];

(async () => {
    console.log("投 Starting Review Acquisition Verification...");
    console.log("---------------------------------------------------");

    for (const productName of PRODUCTS) {
        console.log(`\n逃 Processing: ${productName}`);
        ...
        try {
            ...
            if (kakakuResult && kakakuResult.summary) {
                ...
                console.log(`   笨・Kakaku: Found ${kakakuCount} reviews in ${kakakuTime.toFixed(1)}s`);
                if (kakakuResult.positive.length > 0) console.log(`      Example (Pos): ${kakakuResult.positive[0].title}`);
            } else {
                console.log(`   笞・・Kakaku: No reviews found`);
            }
        } catch (e) {
        ...
        try {
            ...
            if (productInfo && productInfo.asin) {
                ...
                if (amazonResult && amazonResult.summary) {
                    ...
                    console.log(`   笨・Amazon: Found ${amazonCount} reviews in ${amazonTime.toFixed(1)}s`);
                    if (amazonResult.positive.length > 0) console.log(`      Example (Pos): ${amazonResult.positive[0].title}`);
                } else {
                    console.log(`   笞・・Amazon: No reviews found`);
                }
            } else {
                console.log(`   笞・・Amazon: Product not found (No ASIN)`);
            }
        } catch (e) {
        ...
        console.log(`   統 result: Kakaku=${kakakuCount}, Amazon=${amazonCount}`);
    }

    console.log("\n---------------------------------------------------");
    console.log("笨・Verification Complete");
})();
