...
async function main() {
    console.log("耳 Starting Visual Verification Algorithm...");
    ...
    try {
        ...
        if (noImageText) {
            console.error("笶・FAIL: 'No Image' placeholder detected on Homepage!");
            hasError = true;
        } else {
            console.log("笨・PASS: No placeholders found on Homepage.");
        }
        ...
        if (!heroTitle || heroTitle.trim() === '') {
            console.error("笶・FAIL: Hero section has no title! Frontmatter parsing likely failed.");
            hasError = true;
        }
        ...
        if (brokenImages.length > 0) {
            console.error("笶・FAIL: Found broken images:", brokenImages);
            hasError = true;
        }

        // --- CHECK 1.7: Visual Style Gate (Brightness & Saturation) ---
        console.log("耳 Running Visual Style Gate (Brightness & Saturation)...");
        ...
        if (styleCheckResult.error) {
            console.warn(`笞・・WARN: Could not analyze image style: ${styleCheckResult.error}`);
            ...
            if (styleCheckResult.brightness < 40) {
                console.error("笶・FAIL: Hero image is too DARK for the light theme.");
                hasError = true;
            }
            if (styleCheckResult.saturation < 5) {
                console.error("笶・FAIL: Hero image is B&W (grayscale). Design requires color.");
                hasError = true;
            }
            // Pass
            if (!hasError) console.log("笨・PASS: Image fits visual guidelines.");
        }
        ...
        if (firstPostHref) {
            ...
            if (!ctaVisible) {
                console.warn("笞・・WARN: Amazon CTA text might not be white. Please verify manually.");
            } else {
                console.log("笨・PASS: Amazon CTA button has white text.");
            }
        }

    } catch (e) {
        console.error("笶・FATAL: Verification failed:", e.message);
        hasError = true;
    } finally {
    ...
    if (hasError) {
        console.error("\n圻 Design Verification FAILED.");
        process.exit(1);
    } else {
        console.log("\n笨ｨ Design Verification PASSED.");
        process.exit(0);
    }
}
