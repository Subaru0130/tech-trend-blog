...
function selectDynamicLineup(keywordArg, products) {
    ...
    console.log(`\n🔍 Analyzing Theme for: "${keywordArg}"...`);
    ...
    } else if (keyword.includes('音質') || keyword.includes('music')) {
        console.log("  👉 Theme Detected: [Audiophile] (Prioritizing Sound Quality)");
        weights.sound = 2.0;
    } else if (keyword.includes('会議') || keyword.includes('meeting') || keyword.includes('zoom')) {
        console.log("  👉 Theme Detected: [Work/Zoom] (Prioritizing Mic & Comfort)");
...
console.log(`\n=== 🚀 Starting Full Generation for: ${TARGET_KEYWORD} ===\n`);
...
(async () => {
    try {
        ...
        console.log(`  ✨ SEO Title: ${seoMetadata.title}`);
        ...
        try {
            console.log(`  🎨 Requesting AI Thumbnail for "${TARGET_KEYWORD}"...`);
