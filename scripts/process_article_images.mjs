...
async function checkImageQuality(buffer, productName) {
    ...
    1. **Text Overlays**: Does it have promo text like "Sale", "Free Shipping", "No.1", "Points", or Japanese text like "送料無斁E, "新品E, "ポインチE, "クーポン"?
    2. **Red/Yellow Banners**: Does it have a loud red or yellow frame/banner around the image (often used for sales)?
    3. **Quantity > 1**: Does it show MULTIPLE bottles (set of 2, 3, 6...)? Look for numbers like "2倁E, "3本セチE��", "x6".
...
async function downloadImage(page, product, targetDir) {
    ...
    const negativeKeywords = "-詰め替ぁE-refill -パウチE-セチE�� -大容釁E-業務用 -ケース -箱 -中古";

    // Query: Trust Site Search with "Main Unit" focus
    let query = `${trustedSites} ${product.name} ボトル 本佁E${negativeKeywords}`;
...
async function main() {
    const mdxPath = path.join(__dirname, '../content/posts/2025-12-06-市販シャンプ�E.mdx');
