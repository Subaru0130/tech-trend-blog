...
export async function verifyProducts(productNames) {
    ...
    for (const name of productNames) {
        try {
            ...
            if (asinResult) {
                console.log(`笨・Verified ASIN: ${name} -> ${asinResult.asin}`);
                ...
                try {
                    ...
                    const isDead = await page.evaluate(() => {
                        ...
                        return title.includes('繝壹・繧ｸ縺瑚ｦ九▽縺九ｊ縺ｾ縺帙ｓ') || title.includes('404');
                    });

                    if (response.status() === 404 || isDead) {
                        console.warn(`笞・・ASIN Link Dead: ${asinResult.url} -> Falling back to Search`);
                ...
                if (bestImage) {
                    ...
                    console.warn(`笶・No Image found for ${name}`);
                }

            } else {
                console.warn(`笶・ASIN Not found for ${name}`);
            }
...
const SAFE_HERO_IMAGES = {
    'hair dryer': [
        'https://images.unsplash.com/photo-1521590832167-7bcbfaa6381f?q=80&w=1600&auto=format&fit=crop', // Pink/Colorful
    ],
    'water purifier': [
...
export async function getHeroImage(topic) {
    ...
    if (lowerTopic.includes('dryer') || lowerTopic.includes('繝峨Λ繧､繝､繝ｼ')) {
        candidates = SAFE_HERO_IMAGES['hair dryer'];
    } else if (lowerTopic.includes('water') || lowerTopic.includes('purifier') || lowerTopic.includes('豬・ｰｴ')) {
        candidates = SAFE_HERO_IMAGES['water purifier'];
    } else if (lowerTopic.includes('shampoo') || lowerTopic.includes('繧ｷ繝｣繝ｳ繝励・')) {
        candidates = SAFE_HERO_IMAGES['shampoo'];
    } else if (lowerTopic.includes('humidifier') || lowerTopic.includes('蜉貉ｿ')) {
        candidates = SAFE_HERO_IMAGES['humidifier'];
    }

    // Pick random candidate to vary slightly if multiple exist
    const selected = candidates[Math.floor(Math.random() * candidates.length)];
    console.log(`笨・Selected Verified Image: ${selected}`);
    return selected;
}
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
    const testProducts = [
        "繧｢繝ｳ繝峨ワ繝九・ 繝・ぅ繝ｼ繝励Δ繧､繧ｹ繝・,
        "YOLU 繧ｫ繝ｼ繝繝翫う繝医Μ繝壹い"
    ];
    // verifyProducts(testProducts).then(res => console.log(JSON.stringify(res, null, 2)));
    getHeroImage("繝倥い繝峨Λ繧､繝､繝ｼ").then(url => console.log("Hero URL:", url));
}
