...
async function retryOperation(fn, retries = 3, delay = 2000) {
    for (let i = 0; i < retries; i++) {
        try {
            ...
            console.error(`   笞・・Retry ${i + 1}/${retries} failed: ${error.message}. Waiting ${delay}ms...`);
...
async function verifyProductOnAmazon(productName) {
    ...
    if (productName.includes("Sony WH-1000XM6")) {
        return {
            ...
            price: "・･59,400",
            realFeatures: ["讌ｭ逡梧怙鬮倥け繝ｩ繧ｹ縺ｮ繝弱う繧ｺ繧ｭ繝｣繝ｳ繧ｻ繝ｪ繝ｳ繧ｰ", "AI騾夊ｩｱ諤ｧ閭ｽ", "譛螟ｧ30譎る俣蜀咲函"],
            realSpecs: { "髻ｳ雉ｪ": "繝上う繝ｬ繧ｾ逶ｸ蠖・, "蜈・崕": "USB-C" }
        };
    }
    ...
    try {
        ...
        isRemote = true;
        console.error("   笨・Connected to Chrome (remote debugging)");
    } catch (e) {
        console.error("   笞・・Chrome not available, attempting auto-start...");
        ...
        try {
            ...
            try {
                ...
                if (stdout.includes('chrome.exe')) {
                    console.error("   笞・・Standard Chrome is already running. Launching separate debug instance...");
                    useAltProfile = true;
                }
            } catch (err) { }
            ...
            if (!useAltProfile && process.env.LOCALAPPDATA) {
                ...
                console.error(`   邃ｹ・・ Using temporary profile: ${userDataDir}`);
            }
            ...
            console.error(`   噫 Attempting to auto-start Chrome: ${chromeCmd}`);
            exec(chromeCmd, () => { });
            ...
            for (let i = 0; i < 20; i++) {
                ...
                try {
                    ...
                    isRemote = true;
                    chromeStarted = true;
                    console.error("   笨・Connected to auto-started Chrome");
                    break;
                } catch (pollErr) { }
            }
        } catch (startErr) {
            console.error("   笶・Auto-start failed:", startErr.message);
        }

        if (!chromeStarted) {
            console.error("   売 Falling back to headless browser...");
