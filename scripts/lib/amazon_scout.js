...
async function scoutAmazonProducts(keyword, maxCount = 5, pageNumber = 1) {
    console.log(`\n雰・鞘坂凾・・Market Scout: Searching Amazon for "${keyword}" (Page ${pageNumber})...`);
    ...
    try {
        ...
        if (!foundSelector) {
            console.log("   笶・Error: Product list selector not found. Checking page content...");
            ...
            if (content.includes("robot") || title.includes("CAPTCHA")) {
                console.log("   圷 DETECTED CAPTCHA! Amazon is blocking us.");
            }
        ...
        const items = await page.evaluate(() => {
            ...
            nodes.forEach(node => {
                ...
                // Rating & Review Count
                const ratingEl = node.querySelector('span[aria-label*="5縺､譏溘・縺・■"]');
                const reviewCountEl = node.querySelector('span[aria-label*="蛟九・隧穂ｾ｡"]');
                ...
                if (titleEl && imgEl) {
                    ...
                    const junkKeywords = ["繧ｱ繝ｼ繧ｹ", "繧ｫ繝舌・", "菫晁ｭｷ", "case", "cover", "cable", "繧ｱ繝ｼ繝悶Ν", "繝輔ぅ繝ｫ繝", "繧､繝､繝ｼ繝斐・繧ｹ", "skin", "sticker", "ear hooks", "replacement"];
                    ...
                    if (junkKeywords.some(kw => lowerTitle.includes(kw))) {
                        ...
                        // Safest: If "for AirPods" or "逕ｨ" (for) is present with Case/Cover.
                        if (lowerTitle.includes("逕ｨ") || lowerTitle.includes("for") || lowerTitle.includes("compatible with")) return;

                        // If title starts with "Case", it's junk.
                        if (lowerTitle.startsWith("case") || lowerTitle.startsWith("繧ｱ繝ｼ繧ｹ")) return;
                    ...
                    if (modelMatch) {
                        ...
                        if (!lowerTitle.includes(modelId)) {
                            // console.log(`   笞・・Skipping "${title}" - Does not contain model ID "${modelId}"`);
                            return;
                        }
                    } else {
                    ...
                    results.push({
                        ...
                        asin: asin,
                        rating: ratingEl ? parseFloat(ratingEl.getAttribute('aria-label').split('5縺､譏溘・縺・■')[1]) : 4.0,
                        ...
                        specs: [{ label: "萓｡譬ｼ", value: `ﾂ･${priceRaw}` }]
                    });
                }
            });
            return results;
        ...
        console.log(`   笨・Scout found ${items.length} candidates.`);
        await browser.disconnect();
        return items.slice(0, maxCount);

    } catch (e) {
        console.error(`   笶・Scout Error: ${e.message}`);
        await browser.disconnect();
        return [];
    }
}
 ...
 * 蝠・刀隧ｳ邏ｰ繝壹・繧ｸ (/dp/ASIN) 縺九ｉ繝ｬ繝薙Η繝ｼ繧貞叙蠕・- scoutAmazonProducts縺ｨ蜷後§繧｢繝励Ο繝ｼ繝・ * 繧ｷ繝√Η繧ｨ繝ｼ繧ｷ繝ｧ繝ｳ繝ｻ繧ｭ繝ｼ繝ｯ繝ｼ繝峨ｒ蜷ｫ繧繝ｬ繝薙Η繝ｼ繧貞━蜈育噪縺ｫ謚ｽ蜃ｺ
 */
async function scrapeProductReviews(asin, maxReviews = 10) {
    console.log(`\n当 Scraping Reviews for ASIN: ${asin}...`);
    const situationKeywords = ['髮ｻ霆・, '騾壼共', '繧ｫ繝輔ぉ', '繧ｸ繝', '繝ｩ繝ｳ繝九Φ繧ｰ', '鬚ｨ蛻・ｊ髻ｳ', '繧ｪ繝輔ぅ繧ｹ', '鬟幄｡梧ｩ・, '蝨ｰ荳矩延', '莨夊ｭｰ', '繝・Ξ繝ｯ繝ｼ繧ｯ', '蝨ｨ螳・];

    // HELPER: Scrape logic to be reused for initial attempt and retry
    const doScrape = async (isRetry = false) => {
        console.log(isRetry ? `   売 Retry Attempt: Connecting to existing Chrome...` : `   噫 Initial Attempt: Connecting to existing Chrome...`);
        ...
        try {
            ...
            console.log(`   伯 WebSocket URL: ${wsUrl}`);
            browser = await puppeteer.connect({
                browserWSEndpoint: wsUrl,
                defaultViewport: null
            });
            console.log("   笨・Connected to existing Chrome instance!");
        } catch (e) {
            console.log(`   笞・・Failed to connect to Chrome: ${e.message}`);
            console.log("   噫 Attempting to auto-start Chrome with remote debugging...");
            ...
            try {
                ...
                if (isChromeRunning) {
                    console.log('      笞・・Standard Chrome is running but not debuggable. Restarting in Debug Mode...');
                    console.log('      八 Closing existing Chrome processes (with retries)...');
                    ...
                    console.log('      笨・Chrome processes terminated.');
                }
                ...
                console.log("   噫 Launching Chrome via scripts\\start_chrome_quiet.bat...");
                const batPath = path.join(__dirname, '..', 'start_chrome_quiet.bat'); // Note: ../ relative path
                try {
                    execSync(`"${batPath}"`, { stdio: 'inherit' });
                } catch (e) {
                    console.log(`   笞・・Startup script warning: ${e.message}`);
                }
                ...
                console.log("   竢ｳ Waiting for Chrome to start (timeout: 60s)...");
                for (let i = 0; i < 120; i++) {
                    ...
                    try {
                        ...
                        console.log(`   伯 Chrome started! WebSocket URL: ${wsUrl}`);
                        browser = await puppeteer.connect({
                            browserWSEndpoint: wsUrl,
                            defaultViewport: null
                        });
                        console.log("   笨・Connected to auto-started Chrome!");
                        chromeStarted = true;
                        break;
                    } catch (pollErr) {
                        // Still waiting...
                ...
                console.log(`   笞・・Chrome auto-start failed: ${startErr.message}`);
            }

            // Final fallback: headless browser -> REMOVED to avoid Login Wall
            if (!chromeStarted) {
                console.log("   笶・Failed to connect to Remote Chrome. Aborting review scrape to avoid Login Wall.");
                console.log("   笞・・Please ensure Chrome is running with: --remote-debugging-port=9222");
                throw new Error("Remote debugging connection failed");
                /* 
                console.log("   売 Falling back to internal headless browser...");
                try {
                    ...
                    console.error("   笶・Failed to launch fallback browser:", launchError.message);
        ...
        try {
            ...
            console.log(`   匠 Warmup: Visiting Amazon Home Page (wait ${warmupWait}ms)...`);
            try {
                ...
                console.log(`      笞・・Warmup issue: ${e.message}`);
            }
            ...
            if (!isRetry) {
                ...
                console.log(`   迫 Fetching review page (Direct): ${reviewUrl}`);
                try {
                    await page.goto(reviewUrl, { waitUntil: 'domcontentloaded', timeout: 30000 }).catch(e => console.log(`      笞・・Nav timeout ignored`));
                    const currentUrl = page.url();
                    const pageTitle = await page.title();
                    if (currentUrl.includes('signin') || pageTitle.includes('Sign-In') || pageTitle.includes('繝ｭ繧ｰ繧､繝ｳ')) {
                        console.log("   圷 Redirected to Sign-In page! Blocking detected.");
                        blocked = true;
                    } else {
                        ...
                        try { await page.waitForSelector('[data-hook="review"]', { timeout: 8000 }); } catch (e) {
                            console.log("      笞・・Review selector not found immediately, continuing anyway...");
                        }
                    }
                } catch (e) {
                    console.log(`      笞・・Nav error: ${e.message}`);
                }
            } else {
                // Retry: Go via Product Page (Natural flow)
                console.log(`   迫 Fetching review page (Via Product Page): https://www.amazon.co.jp/dp/${asin}`);
                try {
                    ...
                    if (seeAll) {
                        console.log("   迫 Following 'See All Reviews' link...");
                        await Promise.all([
                            page.waitForNavigation({ waitUntil: 'domcontentloaded', timeout: 30000 }),
                            seeAll.click()
                        ]).catch(() => console.log("   笞・・Click nav timeout"));

                        // Check for block again
                        const currentUrl = page.url();
                        if (currentUrl.includes('signin')) {
                            console.log("   圷 Redirected to Sign-In after click!");
                            blocked = true;
                        } else {
                            navSuccess = true;
                            try { await page.waitForSelector('[data-hook="review"]', { timeout: 8000 }); } catch (e) { }
                        ...
                        console.log("   笞・・'See all reviews' link not found on Product Page.");
                        // Treat as not blocked, but standard fallback will handle scraping from main page
                    }
                } catch (e) { console.log(`      笞・・Indirect nav failed: ${e.message}`); }
            }
            ...
            if (navSuccess && !blocked) {
                while (allReviews.length < maxReviews && currentPage <= maxPages) {
                    ...
                    console.log(`      塘 Page ${currentPage}: Found ${pageReviews.length} reviews`);
                    ...
                    } catch (e) { console.log("      笞・・Next button search error"); }
                    console.log(`      剥 Next button found: ${!!nextButton}`);
                    if (nextButton) {
                        currentPage++;
                        console.log(`      塘 Navigating to Page ${currentPage}...`);
                        try {
                            await Promise.race([
                                Promise.all([
                            ...
                            console.log(`      笨・Page ${currentPage} loaded`);
                        } catch (e) {
                            console.log(`      笞・・Pagination nav issue: ${e.message}. Stopping pagination.`);
                            break;
                        }
                    } else {
                        console.log("      塘 No more pages available");
                        break;
                    }
                }
            }
            ...
            if (allReviews.length === 0 && !blocked) {
                console.log("   笞・・No reviews on dedicated page. Falling back to Product Page...");
                await page.goto(`https://www.amazon.co.jp/dp/${asin}`, { waitUntil: 'domcontentloaded', timeout: 45000 }).catch(() => { });
                await new Promise(r => setTimeout(r, 2000));
                ...
                if (seeAll) {
                    console.log("   迫 Following 'See All Reviews'...");
                ...
                console.log(`      塘 Fallback found ${allReviews.length} reviews`);
            }
    ...
    if (result.blocked || result.reviews.length === 0) {
        console.log(`   笞・・Initial scrape failed (Blocked: ${result.blocked}, Count: ${result.reviews.length}). Initiating RETRY with enhanced warmup...`);
    ...
    console.log(`   笨・Reviews scraped: ${output.summary.totalFound} total`);
    return output;
}
/**
...
const CATEGORY_KEYWORDS = {
    'wireless-earphones': ['繧､繝､繝帙Φ', '繧､繝､繝輔か繝ｳ', 'earphone', 'earbuds', 'earbud', '繝ｯ繧､繝､繝ｬ繧ｹ', 'tws', 'buds'],
    'wireless-headphones': ['繝倥ャ繝峨・繝ｳ', '繝倥ャ繝峨ヵ繧ｩ繝ｳ', 'headphone', 'headset', '繧ｪ繝ｼ繝舌・繧､繝､繝ｼ'],
    'bone-conduction': ['鬪ｨ莨晏ｰ・, 'bone', 'shokz', '繧ｪ繝ｼ繝励Φ繧､繝､繝ｼ', 'open ear', 'openrun']
};
...
async function verifyProductOnAmazon(productName, category = 'wireless-earphones') {
    console.log(`   剥 Verifying on Amazon: "${productName}"`);
    ...
    try {
        ...
        await page.goto(searchUrl, { waitUntil: 'domcontentloaded', timeout: 30000 }).catch(e => console.log(`      笞・・Nav timeout ignored (search)`));

        // Ensure results are present before evaluating
        try {
            await page.waitForSelector('div[data-asin]', { timeout: 10000 });
        } catch (e) {
            console.log(`      笞・・Results selector timeout (continuing anyway)`);
        }
        await new Promise(r => setTimeout(r, 1000));
        ...
        let result = await page.evaluate((expectedName, catKeywords) => {
            ...
            for (const node of nodes) {
                ...
                // Exclude junk
                const junkKeywords = ["繧ｱ繝ｼ繧ｹ逕ｨ", "繧ｫ繝舌・", "菫晁ｭｷ", "case for", "cover for", "cable", "繧ｱ繝ｼ繝悶Ν", "繝輔ぅ繝ｫ繝", "繧､繝､繝ｼ繝斐・繧ｹ"];
                if (junkKeywords.some(kw => lowerTitle.includes(kw))) continue;

                return {
                    amazonTitle: title,
                    asin: asin,
                    price: priceEl ? `ﾂ･${priceEl.innerText}` : null,
                    image: imgEl ? imgEl.src : null,
                    link: `https://www.amazon.co.jp/dp/${asin}`,
                    rating: ratingEl ? parseFloat(ratingEl.getAttribute('aria-label').split('5縺､譏溘・縺・■')[1]) : null,
                    reviewCount: reviewCountEl ? parseInt(reviewCountEl.getAttribute('aria-label').replace(/[^0-9]/g, '')) : 0
                };
            }
            return null;
        ...
        if (result) {
            console.log(`      逃 Amazon result: ${result.amazonTitle.slice(0, 60)}...`);

            // AI Verification
            const isMatch = await verifyProductMatchWithAI(productName, result.amazonTitle);

            if (isMatch) {
                console.log(`      笨・AI confirmed: Correct product`);
                result.name = productName;
                return result; // Return immediately
            } else {
                console.log(`      笞・・AI rejected: Not the same product`);
        ...
        if (modelNumberMatch && !result) {
            ...
            if (bestCandidate && bestCandidate !== productName && bestCandidate.length >= 5) {
                console.log(`      笞・・Amazon verification retry: searching for model number "${bestCandidate}"...`);
                ...
                try {
                    ...
                    console.log(`      笞・・Retry search nav timeout ignored`);
                }
                await new Promise(r => setTimeout(r, 2000));
                ...
                for (const node of retryNodes) {
                    ...
                    if (title.toLowerCase().includes(bestCandidate.toLowerCase())) {
                        console.log(`      笨・Retrieval successful with model number: ${title.slice(0, 50)}...`);
        ...
        console.log(`      笶・Not found on Amazon`);
        return null;

    } catch (e) {
        console.error(`      笶・Amazon verification error: ${e.message}`);
        return null;
    } finally {
...
async function verifyProductMatchWithAI(marketResearchName, amazonTitle) {
    ...
    const prompt = `
陬ｽ蜩√・繝・メ繝ｳ繧ｰ遒ｺ隱阪ち繧ｹ繧ｯ:

蟶ょｴ隱ｿ譟ｻ縺ｧ隕九▽縺九▲縺溯｣ｽ蜩∝錐: "${marketResearchName}"
Amazon縺ｮ讀懃ｴ｢邨先棡繧ｿ繧､繝医Ν: "${amazonTitle}"

雉ｪ蝠・ 縺薙ｌ繧峨・蜷後§陬ｽ蜩√〒縺吶°・・
蛻､譁ｭ蝓ｺ貅・
- 繝悶Λ繝ｳ繝牙錐縺御ｸ閾ｴ縺吶ｋ縺・- 繝｢繝・Ν逡ｪ蜿ｷ/蝙狗分縺御ｸ閾ｴ縺吶ｋ縺・- 濶ｲ驕輔＞繧・ｮｹ驥城＆縺・・縲悟酔縺倩｣ｽ蜩√阪→縺吶ｋ
- 蜈ｨ縺丞挨縺ｮ繝｢繝・Ν・井ｾ・ Ear(a) vs Ear(2)・峨・縲悟挨陬ｽ蜩√・
蝗樒ｭ泌ｽ｢蠑・ "YES" 縺ｾ縺溘・ "NO" 縺ｮ縺ｿ
`;
    ...
    try {
        ...
        console.log(`      笞・・AI verification failed: ${e.message}`);
...
async function scrapeAmazonProductSpecs(asin) {
    ...
    console.log(`   搭 Scraping Amazon specs for ASIN: ${asin}`);
    ...
    try {
        ...
        const data = await page.evaluate(() => {
            ...
            const structured = {
                brand: specs.find(s => s.label.match(/Brand|繝悶Λ繝ｳ繝榎繝｡繝ｼ繧ｫ繝ｼ/i))?.value,
                modelName: specs.find(s => s.label.match(/Model Name|繝｢繝・Ν蜷・i))?.value,
                modelNumber: specs.find(s => s.label.match(/Model Number|Item model number|蝙狗分|繝｢繝・Ν逡ｪ蜿ｷ|驛ｨ蜩∫分蜿ｷ/i))?.value,
            };
        ...
        if (data.specs.length > 0 || data.features.length > 0 || data.image) {
            console.log(`      笨・Found ${data.specs.length} specs, ${data.features.length} features from Amazon`);
            // Clean up structured data
            if (data.structured) {
                if (data.structured.brand) console.log(`      捷・・ Brand: ${data.structured.brand}`);
                if (data.structured.modelName) console.log(`      捷・・ Model: ${data.structured.modelName}`);
            }
            if (data.image) {
                console.log(`      名・・ Image found: ${data.image.slice(0, 50)}...`);
            }
            return {
                specs: data.specs,
        ...
        console.log(`      笞・・No specs found on Amazon page`);
        return null;

    } catch (e) {
        console.log(`      笞・・Amazon spec scrape failed: ${e.message}`);
