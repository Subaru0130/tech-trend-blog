    ...
    return new Promise((resolve, reject) => {
        ...
        https.get(url, (response) => {
            if (response.statusCode !== 200) {
                ...
                console.error(`笶・Failed to download image: ${response.statusCode} ${response.statusMessage} for ${url}`);
            ...
            file.on('finish', async () => {
                file.close(async () => {
                    ...
                    try {
                        ...
                        // console.log(`   笨ｨ Meta-injected: ${filename}`);
                        resolve(publicPath); // Return the web-accessible path
                    } catch (e) {
                        console.warn(`   笞・・Metadata injection failed for ${filename}:`, e.message);
            ...
            fs.unlink(filepath, () => { });
            console.error(`笶・Error downloading image ${url}:`, err.message);
            resolve(null);
        });
    });
}
