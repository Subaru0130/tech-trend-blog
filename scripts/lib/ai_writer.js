...
async function generateBuyingGuideBody(keyword, productList, blueprint, stats = { totalScanned: 50, finalCount: 10 }) {
    ...
    console.log(`\n🤖 Generating Buying Guide for "${keyword}"...`);
...
- 読者の状況/悩み: ${ctx.target_reader_situation}
- 比較・評価の軸: ${ctx.comparison_axis}
- 記事のゴール(Sales Hook): ${ctx.sales_hook}

# 使用する商品リスト
${productListString}

# 厳守ルール
⭕️ **「〜です・〜ます」調で統一する。** 読者に語りかけるような丁寧なトーンで。

❌ 「〜だ」「〜である」調（タメ口・論文調）は禁止。
❌ 「断定的に書け」という古い指示は忘れること。ただし「〜だと思います」ばかりの曖昧な表現も避ける。「〜と言えます」「〜が好評です」など、データに基づく確信を持って語る。

# 追加ルール
- 1文は長すぎないように（読点「、」を適切に使う）
- 専門用語は初出時にカッコで解説（例：「LDAC（ソニーの高音質規格）」）

# 【日常語ルール】スペック値は必ず「生活の中の体験」に言い換えろ（全カテゴリ共通）

スペック値を記事に書く際、JIS規格の正式用語や専門的な数値をそのまま使わないでください。
読者が「で、結局どういうこと？」と思わないよう、**日常的な体験に置き換えて説明**してください。
このルールはイヤホンに限らず、家電・家具・カメラ・PC周辺機器など**すべてのカテゴリで適用**してください。

変換ルール：スペックの数値や用語 → 「つまり日常生活ではどういうことか」を必ず添える。

| ❌ そのままの専門用語 | ⭕️ 日常語に変換した例 |
|---|---|
| 飛沫に対する保護 | 汗や小雨くらいなら平気 |
| 噴流水に耐えられる | シャワーの水が直接かかっても大丈夫 |
| 一時的に水中に沈めても浸水しない | うっかり水に落としても壊れない |
| 常時水中での使用が可能 | プールや水泳中でも使える |
| 粉塵の侵入を完全に防護 | 砂浜やホコリの多い場所でも安心 |
| 周波数特性 20Hz-20kHz | 人間の耳で聞こえる全範囲をカバー |
| インピーダンス 32Ω | スマホ直挿しでも十分な音量が取れる |
| 耐荷重 150kg | 体重80kgの人が座っても余裕あり |
| 消費電力 1200W | 電子レンジ1台分くらいの電気を使う |
| 騒音レベル 45dB | 図書館くらいの静かさ |

**重要**: 上記は一例です。記事に登場するあらゆるスペック値に対して「つまり、読者の日常生活ではどういう意味なのか」を必ず補足してください。数値だけ書いて放置するのは禁止です。

# 【絶対禁止】AIっぽいフレーズ（使うな）

以下の表現は「AIが書いた感」が強いため、絶対に使用しないでください。
...
| 〜を最大化 | 〜がすごい / 〜を活かせる |
| 〜を体験できます | 〜が味わえる |
| 幸せになれます | 満足できます / 後悔しません |
| 〜に寄り添う | 〜向けの |
| 至高の | 最高の / 一番の |
| 多くのレビューから見えてくる | 意外と多いのが〜 / ありがちなのが〜 |
| 一部のユーザーは | 〜派の人は / 〜が気になる人は |
| レビューを分析すると | （削除：言わなくていい） |
| 口コミ傾向として | （削除：言わなくていい） |

**例外**: 「ユーザーからの評価は？」セクションのみ、レビュー分析スタンスでOK。それ以外では分析っぽさを消すこと。
...
❌ AI: 「iPhoneユーザーにとって、シームレスな接続を実現します」
...
**【本文全体のマーカールール（絶対厳守）】**:
- 各セクションの本文中で、読者が「ここだけ読めば要点がわかる」重要フレーズを **1セクションにつき2〜4箇所** <mark>...</mark> タグで囲んでください。
- まとめセクションだけでなく、**セクション1〜3の本文中にも必ず入れること**。
- 対象: 結論・数値の基準・失敗パターンの核心など、斜め読みする読者の目に留まるべきフレーズ。
- ❌ 悪い例: 単語1つだけ → <mark>ノイキャン</mark>が重要
- ⭕️ 良い例: フレーズ → <mark>ノイキャンONの状態で単体6時間以上もつモデル</mark>が合格ライン
- ⭕️ 良い例: <mark>1万円を超えると急に性能が上がる「壁」が存在</mark>します
...
1. **読者の悩みへの共感（戦略的ターゲットへの直球）**（2-3段落）
   ...
   - ❌ 禁止: **「現役〇〇の声を参考に」**（そのデータがない場合は嘘になる）。
   - ⭕️ 推奨: 「毎朝の地下鉄の轟音。せっかくの自分の時間が台無しになっていませんか？」（多くの人に刺さる表現）
   - ⭕️ 推奨: 主観的な魅力（「汗に強い」「ズレない」）を入れてOK。ただし「〇〇を実現します」ではなく「〇〇なモデルを厳選しました」形式で。
   - その状況でなぜ従来の製品では満足できないのかを指摘する。

2. **この記事の価値**（1-2段落）
   - 「${ctx.sales_hook}」を読者への約束として提示
   - どんな基準で商品を評価したか
   - **「レビューを調べてわかったこと」を自然に入れる**（例：「このカテゴリで“汗で滑る”という声が意外と多かった」）
...
- ⭕️ **「[Verified] Spec Reality」**や**「[Verified] Best Scenario」**の情報を引用し、「スペック上は〇〇だけど、実際は△△」という形で書く。
- 「調べていく中でわかったこと」を自然に共有する。
- 「〜って人が多い」「〜が気になる人もいる」という形で書く。
...
- 「専門家のおすすめ」ではなく「比較してみた結果」として説得力を出す。
...
3. **ユーザーのリアルな反応（自然な文脈で）**
   - 「口コミ分析でわかった」と書かず、「〜という声もある」「特に〜を気にする人からは」といった形で、解説の中に自然にユーザーの声を織り交ぜる。
...
## セクション4: 結論（迷っている読者の背中を押す）
**見出し**: 「## まとめ：結局どれがいい？」

### 【絶対厳守】選び方セクションとの用語整合性
結論で製品を推薦する際に使うスペック用語・規格名・基準値は、**セクション2（今回の比較ポイント）で説明した体系・表現と必ず一致させてください。**
選び方で使っていない用語や表記をいきなり出さないこと。読者は選び方の知識を前提に結論を読みます。
...
3. **ランキングへの導入**（1文のみ）
   - 最後に「それでは、今回おすすめする${stats.finalCount}製品をランキング形式で紹介します。」と書いてセクションを締める。
...
⭕️ OK例（ジム利用者向け）: 「バッテリー8時間あれば、1時間のトレーニング×週7でも1週間は充電不要」
⭕️ OK例（通勤者向け）: 「バッテリー8時間あれば、往復2時間の通勤×4日は充電なしでOK」
...
async function generateReviewBody(product, competitorName, blueprint = {}) {
    ...
    const formatReviews = (reviews, count, charLimit) => {
        ...
        return reviews
            ...
            .slice(0, count)
            .map(r => `「${r.text.slice(0, charLimit)}${r.text.length > charLimit ? '...' : ''}」`)
            .join('\n');
    };
...
## 参考情報：ユーザーの声（Deep Analysis Data）
【重要】以下は実際の購入者による生の声です。データ量が多いため、これを活用して**「記事の深み（文字数・具体性）」**を出してください。
レビューから「意外な発見」や「具体的な使用感」が見つかった場合は、それを積極的に記事に盛り込んでください。（無理にシーンを限定・捏造する必要はありません）
...
## この製品の実力は？
   - この製品の強みと弱みを具体的に描写。
   - 【重要】特定の用途（ジム、通勤、テレワーク等）に限定せず、製品の一般的な特徴を中立的に評価する。
...
## ⚠️ 致命的な欠点 (Deal Breaker)
   ...
   - **内容**: 「こういう人は注意が必要」という条件を明確にする。
   - **書き方（厳守）**: 
     - 編集部としての「良心」を示す最重要パート。メーカーへの忖度は禁止。
     - **【スペックの嘘を暴く】**: 公称スペックと実測/体感に乖離がある場合はここで指摘してください。（例：「公称30時間だが、NCオンだと実質20時間程度」など）
     - ❌ 「少し高いかも」
     - ⭕️ 「『最強ノイキャン』を探しているなら、この機種の強度は中程度なので満足できない可能性が高い」
     - ⭕️ 「耳の穴が小さい人には物理的に入らない可能性がある」「遅延が0.2秒あるため、**FPSゲーマーは絶対に勝てない**」
     - ⭕️ 「iPhoneユーザーは機能の半分が使えないのでAirPodsのほうがおすすめできます」
...
## この製品の実力は？
   - この製品の強みと弱みを具体的に描写。
   - 【重要】特定の用途（ジム、通勤、テレワーク等）に限定せず、製品の一般的な特徴を中立的に評価する。
   - **【最重要】文章量を十分に確保してください（400文字以上）。**
   - 3〜4段落構成で、「音質・画質」「機能性（ノイキャン等）」「使い勝手（装着感・操作性）」などの観点を網羅的に詳しく解説する。
   - 結論として「S評価レベル」なのか「注意が必要」なのかをはっきりさせる。

## ユーザーからの評価は？
   **【最重要】このセクションは情報量を惜しまず書いてください。レビューから得られた情報は全て活用すること。**

   🙆‍♂️ **評価されている点**（3〜5項目、各項目2〜3文で詳しく）:
   - **必ず以下のレビューデータを参照して具体的に書く**:
     - Amazon/Kakaku.comのPositive Reviewsを確認。
   - 「ノイキャンが自然」「装着感が軽い」「接続が安定」など、レビューで頻出するポイントを自然に引用。
   - 各ポイントについて「なぜ評価されているか」の理由も添える。
   - 例：「多くのユーザーが『つけているのを忘れる軽さ』を挙げています。特に長時間使用する人からの評価が高いです。」

   🙅‍♂️ **不満な点**（2〜4項目、各項目2〜3文で詳しく）:
   - **必ず以下のレビューデータを参照して具体的に書く**:
     - Amazon/Kakaku.comのNegative Reviewsを確認。
   - 「イヤーピースが合わない」「ケースが大きい」「低音が弱い」など具体的な不満を引用。
   - 各ポイントについて「どんな人が不満を感じやすいか」も添える。
   - 例：「『低音が物足りない』という意見が見られます。重低音を重視する人は注意が必要です。」

   📝 **補足**（1〜2段落）:
   - 低評価の原因を深掘り。誤解やミスマッチが原因なら指摘する。
   - 例：「『音質が悪い』という意見は、イヤーピースのサイズが合っていないケースがほとんどです。」
   - 例：「『接続が切れる』という意見は、古いファームウェアの時期に集中。現在は改善されています。」

## 使用感は？
   - **[Verified] Spec Analysis** のデータを基に記述。
   - 例：「バッテリー30時間とありますが、ノイキャンONの実測は22時間程度です。」
   - 例：「『世界最小級』とありますが、耳の小さい人には厚みが気になる可能性があります。」
   - 良いギャップ（例：思ったより充電が早い）があればそれも記述。

## 競合製品との比較
   - \${competitorName} や同価格帯のライバルと比較。
   - 「音質ならA、機能ならこの製品」といった明確な使い分けを提示。
   - 具体的な製品名を出して比較しても良い（わかれば）。

## おすすめする人・しない人
   **【マーカー必須】** 各おすすめ文の冒頭フレーズ（例：「通勤電車での静寂を求めるなら」）には必ず <mark>...</mark> を使用してください。

   "### この製品がおすすめな人"
   - <mark>具体的なライフスタイルや価値観</mark>を3パターン提示。
   - 「〜が好きな人」だけでなく「〜な環境で使う人」など具体的に。

   "### この製品をおすすめできない人"
   - **重要：ここで信頼性を勝ち取る。** 忖度なしで書く。
   - <mark>〜な人</mark>は、他社製品のほうが満足度が高いです。
   - 例：<mark>重低音の迫力を最重視する人</mark>（JBLのほうが向いているため）。
`;
...
# Role
**徹底的な製品リサーチャー・分析官**
この製品を**膨大なユーザー口コミとスペックに基づき**分析・評価してください。

【最重要】汎用レビューのルール:
- 特定の用途（ジム、通勤、テレワーク等）に限定した文脈で書かないでください。
- 「ジムで」「通勤中に」「テレワークで」などの限定的なシーン描写は禁止です。
- 製品の一般的な特徴・性能を中立的に評価してください。
- このレビューは様々なランキング記事から参照されるため、汎用性が必要です。
...
**【論理の飛躍防止：スペック用語の解説ルールの徹底（超重要）】**
- 記事の途中の製品紹介だけを読んだ読者でも理解できるよう、**スペック値や専門用語（英数字の規格名など）が登場した場合は、初心者向けに「それがどういう意味を持ち、どう優れているのか」を数十字で必ず補足**してください。
- 前提知識をすっ飛ばして「〇〇規格対応でおすすめ！」とだけ書くのは論理の飛躍（論理破綻）です。常に読者の目線を意識し、納得できる論理展開を心がけてください。

# 【日常語ルール】スペック値は必ず「生活の中の体験」に言い換えろ（全カテゴリ共通）

スペック値を記事に書く際、専門的な数値や用語をそのまま使わないでください。
読者が「で、結局どういうこと？」と思わないよう、**日常的な体験に置き換えて説明**してください。
...
| 一時的に水中に沈めても浸水しない | うっかり水に落としても壊れない |
| 周波数特性 20Hz-20kHz | 人間の耳で聞こえる全範囲をカバー |
| インピーダンス 32Ω | スマホに直接つないでも十分な音量 |
| 耐荷重 150kg | 体重80kgの人が座っても余裕あり |
| 騒音レベル 45dB | 図書館くらいの静かさ |

**重要**: あらゆるスペック値に対して「読者の日常ではどういう意味か」を必ず補足すること。
...
- **評価との整合性（最重要）**:
    ...
    - 具体的には、自分の評価を述べた後に「実際に使用しているユーザーからも同様の声が多い」「一方でこんな意見もある」という形で**分析結果**を挿入します。
    - ❌ **直接引用や、個人の感想を捏造する「擬似的な鍵カッコ」書きは完全に禁止**です。
    - ❌ **「約2割」「30%」のような具体的な数値/割合の捏造は禁止**（統計データがないため）。
    - ⭕️ 「多くのユーザー」「一部のユーザー」「〜という声が散見されます」といった**定性的な表現**に留めてください。
    
    \`\`\`html
    <div className="review-summary">
        <p><strong>🙆‍♂️ 評価されている点：</strong>多くのユーザーが「ノイキャンの自然さ」を挙げており、特に〜という声が目立ちます。</p>
        <p><strong>🙅‍♂️ 不満な点：</strong>一部のユーザーから「ケースの大きさ」に関する指摘があり、〜という意見も見られます。</p>
    </div>
    \`\`\`
    
    - **使い方の例**:
      - メリットの裏付け: 「ノイキャン性能は非常に高く、実際の購入者からも『電車の音が消える』という報告が多数寄せられています」
      - デメリットの補足: 「一方で、装着感については『長時間だと痛い』という意見も一部で見られます」
