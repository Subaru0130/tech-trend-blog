...
async function scrapeProductSpecs(productName, asin = null) {
    ...
    if (asin) {
        ...
        if (specs && (specs.specs.length > 0 || specs.features.length > 0)) {
            console.log(`      ✁EGot real specs from Amazon!`);
            return specs;
        }
    }
    ...
    console.log(`      ⚠�E�ENo structured specs found, using web search fallback`);
...
async function scrapeKakakuSpec(productName) {
    ...
    try {
        ...
        console.log(`      ⚠�E�E価格.com scrape failed: ${e.message}`);
        return null;
    }
}
...
async function scrapeOfficialSite(productName) {
    ...
    try {
        ...
        const query = brandName
            ? `${brandName} ${productName} 公弁E仕槁Eスペック`
            : `${productName} 公弁E仕様`;
        ...
        if (!officialLink) {
            ...
            console.log(`      ⚠�E�ENo official site found for ${brandName || productName}`);
            return null;
        }
        ...
        console.log(`      ⚠�E�EOfficial site scrape failed: ${e.message}`);
        return null;
    }
}
...
async function extractSpecsWithAI(content, productName, source) {
    ...
    const prompt = `
以下�EWebペ�EジコンチE��チE��ら、E{productName}」�Eスペック惁E��を抽出してJSON形式で返してください、E

【厳守ルール、E
1. **「記載なし」「不�E」�E禁止、E* 斁E��から推測するか、一般皁E��同等クラスの値を補完してください、E
2. 「対忁E非対応」等�E頁E��は、忁E��「、E��か「×」で統一すること。「あり」「なし」「対応」�E禁止、E
3. 防水規格は「IPX4」�Eように書くこと。「IPX4相当」などの余計な斁E���E削除、E
4. 連続�E生時間�E「紁E��時間」�E形式に統一。数値+時間の形にする、E

【�E力形式、E
{
    "specs": [
        { "label": "ドライバ�E", "value": "○○mm" },
        { "label": "連続�E生時閁E, "value": "紁E��○時閁E },
        { "label": "ノイズキャンセリング", "value": "、E },
        { "label": "防水規格", "value": "IPX◁E },
        { "label": "Bluetooth", "value": "5.◁E },
        { "label": "対応コーチE��ク", "value": "AAC, SBC" },
        { "label": "重量", "value": "紁E��○g" }
    ],
...
惁E��が見つからなぁE��E��は省略してください、ESONのみを�E力してください、E
`;
    ...
    try {
        ...
        if (jsonMatch) {
            ...
            console.log(`      ✁EAI extracted ${parsed.specs?.length || 0} specs`);
            return parsed;
        }
    } catch (e) {
        console.log(`      ⚠�E�EAI spec extraction failed: ${e.message}`);
    }
...
async function scrapeKakakuReviews(productName, kakakuUrl = null, maxReviews = 10) {
    ...
    try {
        ...
        if (!productPageUrl) {
            ...
            if (!productPageUrl) {
                console.log(`      ⚠�E�EPrimary search failed. Page: ${searchResult.pageTitle} (${searchResult.currentUrl})`);
                console.log(`      ⚠�E�EFound ${searchResult.linkCount} links, but none matched /item/.`);
                ...
                if (modelNumberMatch) {
                    ...
                    if (bestCandidate && bestCandidate.length > 3 && bestCandidate !== productName) {
                        console.log(`      ⚠�E�EKakaku search retry: searching for model number "${bestCandidate}"...`);
            ...
            if (!productPageUrl) {
                await browser.close();
                console.log(`      ⚠�E�ENo product found on 価格.com for: ${productName}`);
                return null;
            }
        }
        ...
        if (!isDirectReviewPage) {
            // Find the review/kuchikomi link (口コチEor レビュー)
        ...
        while (allReviews.length < maxReviews && currentPage <= maxPages) {
            ...
            if (nextLinkExists) {
                ...
                if (!clicked) {
                    console.log('      ⚠�E�ECould not click next link even though it was found.');
                    break;
                }
                ...
                try {
                    ...
                    // console.log(`      ✁ENavigated to: ${newUrl}`);
                    currentPage++;

                } catch (e) {
                    console.log(`      ⚠�E�ENavigation failed (URL didn't change). URL: ${page.url()}`);
                ...
                console.log(`      ⚠�E�EDebug: No next link found. URL: ${page.url()}`);
        ...
        console.log(`      ⚠�E�E価格.com review scrape failed: ${e.message}`);
        return null;
    }
}
