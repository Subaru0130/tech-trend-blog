...
async function main() {
    ...
    if (heroImage) {
        console.error("笶・Hero Image FOUND! (Test Failed)");
        // Log src to be sure
        const src = await page.evaluate(el => el.src, heroImage);
        console.log("Image Src:", src);
    } else {
        console.log("笨・Hero Image NOT found. (Test Passed)");
    }
    ...
    const summaryText = await page.evaluate(() => document.body.innerText.includes('邱ｨ髮・Κ繧ｳ繝｡繝ｳ繝・)); // Check context

    if (summaryText) {
        console.log("笨・Editor Comment found (context confirmed).");
    }
