            ...
            <div className="max-w-7xl mx-auto px-4 md:px-8 relative z-10">
                <div className="text-center mb-16">
                    ...
                    <p className="text-text-sub max-w-2xl mx-auto text-sm leading-relaxed font-medium">
                        音質、装着感、通話品質、バッテリー持ちなど、
                        <br className="hidden md:inline" />
                        毎日の使い方に合わせて比較しやすい人気モデルをまとめています。
                    </p>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start relative max-w-6xl mx-auto">
                    <article className="bg-white rounded-3xl p-6 shadow-soft border border-white hover:border-border-color transition-all duration-300 lg:mt-12 order-2 lg:order-1 h-full flex flex-col relative z-0 group">
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <span className="text-[10px] font-bold text-text-sub bg-surface-subtle px-2.5 py-1 rounded-full border border-border-color">iPhoneと相性◎</span>
                        </div>
                        <div className="aspect-[4/3] rounded-2xl overflow-hidden mb-5 bg-surface-subtle border border-border-color relative">
                            <Image alt={rank2.name} className="object-cover group-hover:scale-105 transition-transform duration-500" fill sizes="(max-width: 1024px) 100vw, 30vw" src={rank2.image} />
                        </div>
                        ...
                        <Link className="block w-full py-3 rounded-xl border border-border-color bg-white text-center font-bold text-sm text-text-main hover:bg-surface-subtle transition-colors" href={`/reviews/${rank2.id}`}>
                            詳しいレビュー
                        </Link>
                    </article>
                    ...
                    <article className="relative bg-white rounded-3xl p-8 shadow-card-hover border border-accent/20 flex flex-col h-full order-1 lg:order-2 z-10 ring-4 ring-accent/5 group">
                        ...
                        <div className="mt-4 flex items-center justify-between mb-6">
                            ...
                            <span className="text-xs text-stone-400 font-medium">更新: 2026.03.20</span>
                        </div>
                        <div className="aspect-video rounded-2xl overflow-hidden mb-6 bg-surface-subtle border border-border-color relative">
                            <Image alt={rank1.name} className="object-cover group-hover:scale-105 transition-transform duration-700" fill sizes="(max-width: 1024px) 100vw, 34vw" src={rank1.image} />
                        </div>
                        ...
                        <div className="mt-auto grid gap-3">
                            <Link className="w-full py-4 rounded-xl bg-primary hover:bg-accent text-white text-center font-bold shadow-lg shadow-primary/20 hover:shadow-accent/30 transition-all flex items-center justify-center gap-2" href={`/reviews/${rank1.id}`}>
                                詳しいレビューを見る <span className="material-symbols-outlined text-[18px]">arrow_forward</span>
                            </Link>
                        </div>
                    </article>
                    ...
                    <article className="bg-white rounded-3xl p-6 shadow-soft border border-white hover:border-border-color transition-all duration-300 lg:mt-12 order-3 lg:order-3 h-full flex flex-col relative z-0 group">
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <span className="text-[10px] font-bold text-text-sub bg-surface-subtle px-2.5 py-1 rounded-full border border-border-color">コスパ重視</span>
                        </div>
                        <div className="aspect-[4/3] rounded-2xl overflow-hidden mb-5 bg-surface-subtle border border-border-color relative">
                            <Image alt={rank3.name} className="object-cover group-hover:scale-105 transition-transform duration-500" fill sizes="(max-width: 1024px) 100vw, 30vw" src={rank3.image} />
                        </div>
                        ...
                        <Link className="block w-full py-3 rounded-xl border border-border-color bg-white text-center font-bold text-sm text-text-main hover:bg-surface-subtle transition-colors" href={`/reviews/${rank3.id}`}>
                            詳しいレビュー
                        </Link>
                    </article>
                </div>
